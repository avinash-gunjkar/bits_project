...
var tag = document.createElement('script');
  tag.src = '../../../../../../www.youtube.com/player_api.js';
...
function onYouTubePlayerAPIReady(){
  tv = new YT.Player('youtube-block', {events: {'onReady': onPlayerReady, 'onStateChange': onPlayerStateChange}, playerVars: playerDefaults});
}
...
function vidRescale(){
  ...
  if (w/h > 16/9){
    tv.setSize(w, w/16*9);
    $('.youtube-block .screen').css({'left': '0px'});
  } else {
    tv.setSize(h/9*16, h);
    $('.youtube-block .screen').css({'left': -($('.youtube-block .screen').outerWidth()-w)/2});
  }
}
