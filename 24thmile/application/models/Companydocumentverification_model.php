...
class Companydocumentverification_model extends CI_Model{

	private $TBL='tbl_company_kyc_documents';
	...
	public function truncate(){			
			return $this->db->truncate($this->TBL);
	}
    ...
    public function getSelectedIdListDetails(){
    	$this->db->select('document_id as value,name');
    	$this->db->join('tbl_document','tbl_document.id = tbl_company_kyc_documents.document_id','left');		
		$query = $this->db->get($this->TBL);
		$rs=array();
		foreach ($query->result_array() as $row){
			array_push($rs, $row);
