...
class Dimension_model extends CI_Model{
	...
	public function getcontainerNameWithDimensionList(){

    	 $this->db->select('tbl_dimension.*, tbl_container.type as container_name');
		$this->db->order_by('id','DESC');
		$this->db->join(' tbl_container',' tbl_container.id = tbl_dimension.container_id','left');
		$this->db->where('tbl_dimension.isActive ', 1);
		$query = $this->db->get($this->TBL);
		$rs=array();
		foreach ($query->result_array() as $row){
			array_push($rs, $row);
