 <fieldset <?php echo ($currentStep->step_id == $stepData[$key]->id) ? 'style="opacity: 1; display: block;"' : '' ?>>
     ...
     <div class="editableDataDiv" style="<?= in_array($shipmentProcessData[$key]->status, ['', '0', '3']) ? 'display:block' : 'display:none' ?>">
       ...
       <div class="col-12 col-lg-3">
           <div class="form-group">
             <label>Reached at Origin Port Date <sup>*</sup></label>
             <input type="text" required="" class="form-control date-picker" required="" placeholder="DD-MM-YYYY" autocomplete="off" name="step3_import_act_date" value="<?=printFormatedDate($shipmentProcessData[$key]->action_date)?>" />
         ...
         <div class="col-12 col-lg-9">
           <div class="form-group">
             <label>Remark</label>
             <input type="text" class="form-control" name="step3_import_details" placeholder="Remark" value="<?= $shipmentProcessData[$key]->corrections ?>">
           </div>
         </div>
   ...
   <input type="hidden" name="step3_import_step_id" value="<?php echo $stepData[$key]->id; ?>">
   <?php if ($bookedShipment->quote_status == 5) { ?>
     <a href="javascript:void(0)" class="btn btn-secondary pull-right mx-2 cancelBtn">Cancel</a>
     <input type="submit" name="step3_import" class="btn btn-success pull-right" value="Submit" style="<?= in_array($shipmentProcessData[$key]->status, ['', '0', '3']) ? 'display:inline-block' : 'display:none' ?>" />
